...
class Settings(BaseSettings):
    ...
    port: int = 8081
    # Сколько апдейтов может одновременно обрабатываться в фоне (см.
    # bot/webhook.py). HTTP-соединение от MAX не удерживается на время
    # обработки — маршрут отвечает сразу, апдейт уходит в фоновую задачу.
    # При переполнении очереди новые апдейты отклоняются 503 (MAX повторит
    # доставку), а не копятся в открытых сокетах до исчерпания дескрипторов.
    webhook_max_pending_updates: int = 500
    # Сколько секунд ждать завершения принятых, но ещё не обработанных
    # апдейтов при остановке бота, прежде чем отменить их принудительно.
    webhook_drain_timeout: float = 20.0
    # Потолок одновременных HTTP-запросов к uvicorn (limit_concurrency): сверх
    # него сервер отвечает 503, а не принимает соединения без предела.
    webhook_limit_concurrency: int = 256
    # Длина очереди на accept (uvicorn backlog). Лишние соединения лучше
    # отбить сразу, чем держать открытыми до таймаута.
    webhook_backlog: int = 256
    # Таймаут keep-alive HTTP-соединений uvicorn, секунды.
    webhook_keepalive_timeout: int = 5
